 ...
 * @extends {Map}
 */
export default class SetMap<K, V, V2 = void> extends Map<K, Set<V> | Map<V, V2>> {
  /**
  ...
  hasMap(key: K, ...values: V[]): boolean {
    ...
    if (this.has(key)) {
      ...
      if (!val || !(val instanceof Set || val instanceof Map)) {
        throw TypeError(
          `[SetMap] | hasMap/hasSet | ERROR | Expected ${String(key)} to be a Map or Set`,
        );
      }
  ...
  sizeMap(key: K) {
    ...
    if (this.has(key)) {
      ...
      if (!(val instanceof Set || val instanceof Map)) {
        throw TypeError(
          `[SetMap] | sizeMap/sizeSet | ERROR | Expected ${String(key)} to be a Map or Set`,
        );
      }
      return val.size;
    }
